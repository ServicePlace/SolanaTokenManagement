...
import type { CustomLogic } from '../types';
import { Code, Save } from 'lucide-react';
import { loadCustomLogic, saveCustomLogic } from '../localStorage';

const CustomLogic: React.FC = () => {
  const [logic, setLogic] = useState<CustomLogic>({
    name: '',
    description: '',
    code: '',
    type: 'reward',
  });
  const [savedLogic, setSavedLogic] = useState<CustomLogic[]>(loadCustomLogic());
  const [isSaving, setIsSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    setSavedLogic(loadCustomLogic());
  }, []);
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setLogic({
                      ...logic,
                      type: e.target.value as CustomLogic['type'],
                    })
                  }
                  className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                >
